...
type BillQuery struct {
	...
	// eager-loading edges.
	withPharmacists       *PharmacistQuery
	withPayments          *PaymentQuery
	withDispenseMedicines *DispenseMedicineQuery
	withFKs               bool
...
// QueryPharmacists chains the current query on the pharmacists edge.
func (bq *BillQuery) QueryPharmacists() *PharmacistQuery {
	query := &PharmacistQuery{config: bq.config}
...
func (bq *BillQuery) QueryPayments() *PaymentQuery {
...
// QueryDispenseMedicines chains the current query on the dispenseMedicines edge.
func (bq *BillQuery) QueryDispenseMedicines() *DispenseMedicineQuery {
	query := &DispenseMedicineQuery{config: bq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := bq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
//  WithPharmacists tells the query-builder to eager-loads the nodes that are connected to
// the "pharmacists" edge. The optional arguments used to configure the query builder of the edge.
func (bq *BillQuery) WithPharmacists(opts ...func(*PharmacistQuery)) *BillQuery {
	query := &PharmacistQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	bq.withPharmacists = query
	return bq
}
...
func (bq *BillQuery) WithPayments(opts ...func(*PaymentQuery)) *BillQuery {
	query := &PaymentQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	bq.withPayments = query
	return bq
}

//  WithDispenseMedicines tells the query-builder to eager-loads the nodes that are connected to
// the "dispenseMedicines" edge. The optional arguments used to configure the query builder of the edge.
func (bq *BillQuery) WithDispenseMedicines(opts ...func(*DispenseMedicineQuery)) *BillQuery {
	query := &DispenseMedicineQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	bq.withDispenseMedicines = query
	return bq
}
...
func (bq *BillQuery) sqlAll(ctx context.Context) ([]*Bill, error) {
	var (
		...
		_spec       = bq.querySpec()
		loadedTypes = [3]bool{
			bq.withPharmacists != nil,
			bq.withPayments != nil,
			bq.withDispenseMedicines != nil,
		}
	)
	if bq.withPharmacists != nil || bq.withPayments != nil || bq.withDispenseMedicines != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := bq.withPharmacists; query != nil {
		...
		for _, n := range neighbors {
			...
			for i := range nodes {
				nodes[i].Edges.Pharmacists = n
			}
		}
	}

	if query := bq.withPayments; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Bill)
		for i := range nodes {
			if fk := nodes[i].payment_id; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "payment_id" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := bq.withDispenseMedicines; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Bill)
		for i := range nodes {
			if fk := nodes[i].dispensemedicine_id; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(dispensemedicine.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "dispensemedicine_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.DispenseMedicines = n
			}
		}
	}

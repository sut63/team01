...
// QueryPrescriptionpatient chains the current query on the prescriptionpatient edge.
func (pq *PrescriptionQuery) QueryPrescriptionpatient() *PatientInfoQuery {
	query := &PatientInfoQuery{config: pq.config}
...
// QueryPrescriptiondoctor chains the current query on the prescriptiondoctor edge.
func (pq *PrescriptionQuery) QueryPrescriptiondoctor() *DoctorQuery {
...
// QueryPrescriptionmedicine chains the current query on the prescriptionmedicine edge.
func (pq *PrescriptionQuery) QueryPrescriptionmedicine() *MedicineQuery {
...
func (pq *PrescriptionQuery) QueryDispensemedicine() *DispenseMedicineQuery {
...
//  WithPrescriptionpatient tells the query-builder to eager-loads the nodes that are connected to
// the "prescriptionpatient" edge. The optional arguments used to configure the query builder of the edge.
func (pq *PrescriptionQuery) WithPrescriptionpatient(opts ...func(*PatientInfoQuery)) *PrescriptionQuery {
	query := &PatientInfoQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withPrescriptionpatient = query
	return pq
}

//  WithPrescriptiondoctor tells the query-builder to eager-loads the nodes that are connected to
// the "prescriptiondoctor" edge. The optional arguments used to configure the query builder of the edge.
func (pq *PrescriptionQuery) WithPrescriptiondoctor(opts ...func(*DoctorQuery)) *PrescriptionQuery {
	query := &DoctorQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withPrescriptiondoctor = query
	return pq
}

//  WithPrescriptionmedicine tells the query-builder to eager-loads the nodes that are connected to
// the "prescriptionmedicine" edge. The optional arguments used to configure the query builder of the edge.
func (pq *PrescriptionQuery) WithPrescriptionmedicine(opts ...func(*MedicineQuery)) *PrescriptionQuery {
	query := &MedicineQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withPrescriptionmedicine = query
	return pq
}
...
//		GroupBy(prescription.FieldValue).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Prescription.Query().
//		Select(prescription.FieldValue).
//		Scan(ctx, &v)
//
...
func (pq *PrescriptionQuery) sqlAll(ctx context.Context) ([]*Prescription, error) {
	...
	if query := pq.withPrescriptionpatient; query != nil {
		...
		for _, n := range neighbors {
			...
			for i := range nodes {
				nodes[i].Edges.Prescriptionpatient = n
			}
		}
	}

	if query := pq.withPrescriptiondoctor; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Prescription)
		for i := range nodes {
		...
		for _, n := range neighbors {
			...
			for i := range nodes {
				nodes[i].Edges.Prescriptiondoctor = n
			}
		}
	}

	if query := pq.withPrescriptionmedicine; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Prescription)
		for i := range nodes {
	...
	if query := pq.withDispensemedicine; query != nil {
		...
		query.Where(predicate.DispenseMedicine(func(s *sql.Selector) {
			s.Where(sql.InValues(prescription.DispensemedicineColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.prescription_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "prescription_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "prescription_id" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Dispensemedicine = n
		}
	}

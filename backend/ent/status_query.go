...
type StatusQuery struct {
	...
	// eager-loading edges.
	withStatusprescription *PrescriptionQuery
...
// QueryStatusprescription chains the current query on the statusprescription edge.
func (sq *StatusQuery) QueryStatusprescription() *PrescriptionQuery {
...
//  WithStatusprescription tells the query-builder to eager-loads the nodes that are connected to
// the "statusprescription" edge. The optional arguments used to configure the query builder of the edge.
func (sq *StatusQuery) WithStatusprescription(opts ...func(*PrescriptionQuery)) *StatusQuery {
	query := &PrescriptionQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withStatusprescription = query
	return sq
}
...
func (sq *StatusQuery) sqlAll(ctx context.Context) ([]*Status, error) {
	...
	if query := sq.withStatusprescription; query != nil {
		...
		query.Where(predicate.Prescription(func(s *sql.Selector) {
			s.Where(sql.InValues(status.StatusprescriptionColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Statusprescription = append(node.Edges.Statusprescription, n)
		}
	}

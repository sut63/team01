...
func (tx *Tx) init() {
	...
	tx.DispenseMedicine = NewDispenseMedicineClient(tx.config)
	tx.Doctor = NewDoctorClient(tx.config)
	tx.DrugAllergy = NewDrugAllergyClient(tx.config)
	tx.LevelOfDangerous = NewLevelOfDangerousClient(tx.config)
	tx.Medicine = NewMedicineClient(tx.config)
	tx.MedicineType = NewMedicineTypeClient(tx.config)
	tx.Order = NewOrderClient(tx.config)
	tx.PatientInfo = NewPatientInfoClient(tx.config)
